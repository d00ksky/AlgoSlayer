...
class OptionsPaperTrader:
    ...
    def close_position(self, prediction_id: str, exit_reason: str) -> bool:
        ...
        logger.success(
            ...
            # Track trade outcome for signal effectiveness learning
            try:
                # Get the position outcome
                cursor = self.conn.cursor()
                cursor.execute('''
                    SELECT net_pnl, pnl_percentage 
                ...
                result = cursor.fetchone()
                if result:
                    net_pnl, pnl_percentage = result
                    outcome = "win" if net_pnl > 0 else "loss"
                    
                    # Update signal effectiveness tracker
                    signal_tracker.update_trade_outcome(
                        prediction_id, outcome, float(net_pnl), float(pnl_percentage)
                    )
                    
                    logger.info(f"📊 Signal effectiveness updated for {prediction_id}: {outcome} ({pnl_percentage:.1f}%)")
                else:
                    logger.warning(f"⚠️ Could not find outcome data for prediction {prediction_id}")
                    
            except Exception as e:
                logger.error(f"❌ Failed to track signal effectiveness: {e}")
